...
Rookery does **not** hardcode locations like `~/.claude/skills/`. Each host profile carries an ordered list of named **workspace paths** (`HostProfile.workspaces: [Workspace]`):

```
[
  { name: "Skills",        path: "~/.claude/skills" },
  { name: "Project notes", path: "~/repos/rookery" },
  { name: "Aider chats",   path: "~/repos/rookery/.aider.chat.history.md" }
]
```
...
- **Running agents themselves.** Rookery is the operator/control surface — viewing and editing the agent's state files, schedules, DBs, and skills. Use Claude Code, Cursor, or your editor of choice to actually run the agent. Local-mode just means we can browse the Mac's own filesystem; we don't host the agent process.
- **General-purpose terminal.** Termius, Tabby, and friends do this well — Rookery's terminal pane (if any) is a fallback, not the headline feature.
- **Cloud orchestration.** No Kubernetes, no AWS console. Rookery assumes long-running boxes the user owns.
- **Windows / Linux client.** Mac-only. Cross-platform may be a v2+ conversation.
- **Hosting / paid SaaS.** This is a local app; there is no Rookery cloud.

## Non-goals (deliberate exclusions)

- Replacing the agent's own UI (e.g., Claude Code's TUI). Rookery complements; it doesn't reimplement.
